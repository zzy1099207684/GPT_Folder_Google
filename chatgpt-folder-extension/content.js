...
// —— 弹出菜单（add group / Export / Import）——
    ...
    `;
            ...
            // ② 新增：Edit 弹窗输入框滚动条与滚轮代理（仅影响弹窗，不影响主输入框）
            function enableEditEditorScroll() {
                const STYLE_ID = 'cgpt-edit-scroll-style';
                if (!document.getElementById(STYLE_ID)) {
                    const s = document.createElement('style');
                    s.id = STYLE_ID;
                    s.textContent = `
      /* 统一给被标记的编辑框或其滚动容器一个可见的竖向滚动条 */
      [role="dialog"] .__cgpt-edit-scroll{
      ...
      [role="dialog"] .__cgpt-edit-scroll::-webkit-scrollbar{ width:10px }
      [role="dialog"] .__cgpt-edit-scroll::-webkit-scrollbar-thumb{ background:rgba(255,255,255,.35); border-radius:8px }
      [role="dialog"] .__cgpt-edit-scroll::-webkit-scrollbar-track{ background:transparent }
    `;
                    document.head.appendChild(s);
                }

                // 选择最合适的可滚动容器（优先最近的可滚动祖先，找不到就用自身）
                const pickScrollable = (el) => {
                    let n = el, best = el, hops = 0;
                    while (n && hops < 4) {
                        if (n.scrollHeight > n.clientHeight + 2) { best = n; break; }
                        n = n.parentElement; hops++;
                    }
                    return best;
                };

                const markDialog = (dlg) => {
                    const editor = dlg.querySelector('textarea,[contenteditable="true"]');
                    if (!editor) return;
                    const box = pickScrollable(editor);
                    if (!box.classList.contains('__cgpt-edit-scroll')) box.classList.add('__cgpt-edit-scroll');

                    // 鼠标滚轮在弹窗任意区域滚动时，优先滚动编辑框
                    if (!dlg.__cgptWheelProxy) {
                        dlg.addEventListener('wheel', (e) => {
                            const target = dlg.querySelector('.__cgpt-edit-scroll');
                            if (!target) return;
                            // 若滚轮不在编辑框内，则把滚动量导向编辑框
                        ...
                        dlg.__cgptWheelProxy = true;
                    }
                };

                const scan = () => {
                    document.querySelectorAll('[role="dialog"][data-state="open"]').forEach(markDialog);
                };
                scan();
                ...
                const mo = new MutationObserver((ms) => {
                    for (const m of ms) {
                        m.addedNodes.forEach(n => {
                            if (n.nodeType !== 1) return;
                            if (n.matches?.('[role="dialog"]')) markDialog(n);
                            n.querySelectorAll?.('[role="dialog"]').forEach(markDialog);
                        });
                    }
                });
                mo.observe(document.body, { childList: true, subtree: true });
